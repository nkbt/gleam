...
AbstractEntity.prototype = {
		...
		if (_.isUndefined(property)) {
			...
			_.each(_.keys(this), function (property) {
				if (!_.isFunction(this[property])) {
					console.log('gety: ' + this.namespace + '.' + property, this[property]);
					getter = 'get' + ucfirst(property);
					if (_.isFunction(this[getter])) {
						data[property] = this[getter](property);
					} else {
	...
	toJSON: function () {
		var data = {};
		_.each(this.get(), function (value, property) {
			if (_.isObject(value) && _.isFunction(value.toJSON)) {
				data[property] = value.toJSON();
			} else {
				data[property] = value;
			}
		}, this);
